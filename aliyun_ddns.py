...
def my_ip(i_ip_ver):
    if i_ip_ver == 4:
        get_ip_value = requests.get('http://v4.ipv6-test.com/api/myip.php', timeout=30).content.decode().strip('\n')
    else:
        get_ip_value = requests.get('http://v6.ipv6-test.com/api/myip.php', timeout=30).content.decode().strip('\n')
    return get_ip_value
